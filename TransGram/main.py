...
'''
Remember, put session logs in client before kick starting Client
Telegram code centre will validate and send verify protocol (code)
to User, 

Also, don't forget to check your Telegram API on my.telegram.org
Telethon require User to filled all factors before start working with client
'''
...
class NotedApp(ft.Column):
    ...
    def Search_Result(self, e):
        ...
        for part in self.Search_List:
            if self.Search_field.value.lower() in part["title"] or self.Search_field.value in part["content"]:
                self.Note_list.controls.append(
...
def main(page:ft.Page):
    ...
    def Search_keyword_result(e):
        ...
        for result in Keyword_list:
            if Search_keyword.value.lower() in result['word'] or Search_keyword.value in result['trans']:
                Wordlist.controls.append(
                    ft.Container(
                            ft.Row(
                                [
...
def main(page:ft.Page):
    ...
    def Upload_language(e):
        Language_convert.options.append(
            ft.dropdown.Option(f"{Language_field.value.lower()}")
        )
        Language_convert.value = Language_field.value
